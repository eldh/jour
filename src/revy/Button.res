...
module Styles = {
  open S
  let base = list{borderRadius(#medium), borderStyle(#solid), borderWidth(3.)}
  let make = (~variant, ~size, ~outline) => {
    let (fontSize, paddingV, paddingH) = switch size {
    | Small => (1, #half, #single)
    | Medium => (2, #single, #double)
    | Large => (3, #double, #triple)
    }
    let (bgColor, borderColor, textColor) = switch (variant, outline) {
    ...
    let text = list{fontWeight(#_700), S.fontSize(fontSize), color(textColor)} |> S.make
    let btn =
      list{
