...
let join = (separator, strings) => {
  let rec run = (acc, strings) =>
    ...
    | list{} => acc
    | list{x} => acc ++ x
    | list{x, ...xs} => run(acc ++ (x ++ separator), xs)
    }
  run("", strings)
}
...
module Converter = {
  ...
  let string_of_hsl = (h, s, l) =>
    "hsl(" ++
    (string_of_angle(h) ++
    (", " ++ (string_of_percent(s) ++ (", " ++ (string_of_percent(l) ++ ")")))))
  ...
  let string_of_stops = stops =>
    stops
    |> List.map(((l, c)) => join(" ", list{string_of_color(c), string_of_length(l)}))
    |> join(", ")
  ...
  let string_of_translate3d = (x, y, z) =>
    "translate3d(" ++
    (string_of_length(x) ++
    (", " ++ (string_of_length(y) ++ (", " ++ (string_of_length(z) ++ ")")))))
...
type color = [#rgb(int, int, int) | #rgba(int, int, int, float) | #transparent]
 ...
 * Units
 ")
let string_of_length_cascading = x =>
  switch x {
...
let flex3 = (~grow, ~shrink, ~basis) =>
  d(
    ...
    (" " ++
    (string_of_float(shrink) ++ (" " ++ string_of_flexBasis(basis)))),
  )
...
let padding2 = (~v, ~h) => d("padding", string_of_length(v) ++ (" " ++ string_of_length(h)))
let padding3 = (~top, ~h, ~bottom) =>
  d(
    "padding",
    string_of_length(top) ++ (" " ++ (string_of_length(h) ++ (" " ++ string_of_length(bottom)))),
  )
let padding4 = (~top, ~right, ~bottom, ~left) =>
  d(
    ...
    (" " ++
    (string_of_length(bottom) ++ (" " ++ string_of_length(left)))))),
  )
...
type gridAutoDirection = [#column | #row | #columnDense | #rowDense | #initial | cascading]

let gridAutoDirectionToJs = x =>
  switch x {
  | #column => "column"
  | #row => "row"
...
let gridAutoFlow = direction => d("gridAutoFlow", gridAutoDirectionToJs(direction))

type repeatValue = [#autoFill | #autoFit | #num(int)]
let repeatValueToJs = x =>
  switch x {
  | #autoFill => "auto-fill"
  | #autoFit => "auto-fit"
...
type trackLength = [length | #fr(float) | #minContent | #maxContent | #minmax(minmax, minmax)]
...
let contentRule = x => d("content", j`"$x"`)
...
let string_of_filter = x =>
  ...
  | #dropShadow(a, b, c, d) =>
    ...
    (" " ++
    (string_of_length(b) ++
    (" " ++ (string_of_length(c) ++ (" " ++ (Converter.string_of_color(d) ++ ")")))))))
 ...
 * Style
 ")
let backfaceVisibility = x => d("backfaceVisibility", string_of_visibility(x))
...
let backgrounds = bg => d("background", bg |> List.map(string_of_background) |> join(", "))
...
let backgroundImage = x =>
  d(
    ...
    | #repeatingLinearGradient(angle, stops) => string_of_repeatingLinearGradient(angle, stops)
    | #radialGradient(stops) => "radial-gradient(" ++ (string_of_stops(stops) ++ ")")
    | #repeatingRadialGradient(stops) =>
      "repeating-radial-gradient(" ++ (string_of_stops(stops) ++ ")")
    },
...
let string_of_listStylePosition = x =>
  switch x {
  | #inside => "inside"
  | #outside => "outside"
  }

let string_of_listStyleImage = x =>
  switch x {
  | #none => "none"
  | #url(url) => "url(" ++ (url ++ ")")
  }
...
let listStyle = (style, pos, img) =>
  d(
    ...
    (" " ++
    (string_of_listStylePosition(pos) ++ (" " ++ string_of_listStyleImage(img)))),
  )
...
let outline = (size, style, color) =>
  d(
    ...
    (" " ++
    (string_of_outlineStyle(style) ++ (" " ++ string_of_color(color)))),
  )
...
let textShadow = (~x=zero, ~y=zero, ~blur=zero, color) =>
  #textShadow(
    string_of_length(x) ++
    (" " ++
    (string_of_length(y) ++ (" " ++ (string_of_length(blur) ++ (" " ++ string_of_color(color)))))),
  )
let string_of_textShadow = x =>
  switch x {
  | #textShadow(s) => s
  }
...
let string_of_transform = x =>
  ...
  | #rotate3d(x, y, z, a) =>
    ...
    (", " ++
    (string_of_float(y) ++
    (", " ++ (string_of_float(z) ++ (", " ++ (string_of_angle(a) ++ ")")))))))
...
let string_of_timingFunction = x =>
  switch x {
  | #linear => "linear"
  | #ease => "ease"
  | #easeIn => "ease-out"
...
let string_of_animationDirection = x =>
  switch x {
  | #normal => "normal"
  | #reverse => "reverse"
  | #alternate => "alternate"
...
let string_of_animationFillMode = x =>
  switch x {
  | #none => "none"
  | #forwards => "forwards"
  | #backwards => "backwards"
...
let string_of_animationIterationCount = x =>
  switch x {
  | #infinite => "infinite"
  | #count(x) => string_of_int(x)
  }
type animationPlayState = [#paused | #running]
let string_of_animationPlayState = x =>
  switch x {
  | #paused => "paused"
  | #running => "running"
  }
