...
module Private = {
  ...
  let fontSize = (~theme, n) => {
    (theme.fontScale ** float_of_int(n) *. float_of_int(theme.baseFontSize))
    ->int_of_float;
  };
  ...
  let space =
      (~negative=false, ~theme, ~adjustPx=0, v): ReactNative.Style.size => {
    let length = v =>
      theme.baseGridUnit * v - adjustPx |> float_of_int |> ReactNative.Style.dp;
    ...
    let va =
      findMinStep(
        i =>
          i
          * theme.baseGridUnit
          |> float_of_int > (size |> fontSize(~theme) |> float_of_int)
          *. 1.25,
        0,
      );
