...
var InvalidValue = Caml_exceptions.create("Lab.InvalidValue");
...
function fromRGB(_param) {
  while(true) {
    var param = _param;
    if (param.HASH >= 5692173) {
      var match = param.VAL;
      _param = {
        HASH: /* rgba */-878128972,
        VAL: [
    ...
    var param$1 = mapTriple((function (param) {
...
function toRGB(param) {
  var match = param.VAL;
  var param$1 = mapTriple(rgbClamp, mapTriple(intOfFloat, mapTriple(srgbGamma, xyzToRGB(d50ToD65(labToXyz([
                            match[0],
                            match[1],
                            match[2]
                          ]))))));
...
function toCss(lab) {
  if (lab.VAL[3] !== 0) {
    ...
    return /* transparent */582626130;
  }
}

function lightness(v, param) {
  var match = param.VAL;
  return {
          HASH: /* lab */5392461,
          VAL: [
...
function highlight(baseColor, factor) {
  ...
  return function (c) {
    return lighten(Math.imul(baseL > 50 && c.VAL[0] + factor > 100 ? -1 : 1, factor), c);
  };
}
...
function getContrastLimit(param) {
  if (param[0]) {
    if (param[1]) {
      return 4.5;
    } else {
